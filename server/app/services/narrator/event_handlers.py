...
def handle_stage_progress_event(
    run_id: str, event_data: Dict[str, Any], state: Optional[ResearchRunState] = None
) -> List[TimelineEvent]:
    """
...
# ============================================================================
# DISPATCHER (Event Type → Handler Function)
# ============================================================================
...
def handle_running_code_event(
    ...
    if state and not is_stage_started(state, stage_name):
        stage_display_name = STAGE_NAMES.get(stage_name, stage_name)
        events.append(
            StageStartedEvent(
                id=str(uuid.uuid4()),
...
# Map raw event types to handler functions
EVENT_HANDLERS = {
...
def process_execution_event(
    run_id: str, event_type: str, event_data: Dict[str, Any], state: Optional[ResearchRunState] = None
) -> List[TimelineEvent]:
    """
