...
async def node_code_experiment(state: State, runtime: Runtime[Context]) -> State:
    ...
    logger.debug(f"experiment_plan: {state.experiment_plan[:32]}")
    logger.debug(f"experiment_code: {state.experiment_code[:32]}")
...
async def node_exec_experiment(state: State, runtime: Runtime[Context]) -> State:
    ...
    logger.debug(f"experiment_stdout: {state.experiment_stdout[:32]}")
    logger.debug(f"experiment_stderr: {state.experiment_stderr[:32]}")
...
async def node_parse_experiment_output(
    ...
    logger.debug(f"experiment_summary: {state.experiment_summary[:32]}")
...
async def node_code_metrics_parser(state: State, runtime: Runtime[Context]) -> State:
    ...
    prompt = prompts.build_prompt_baseline_parser_code(state.experiment_stdout or "")
    ...
    logger.debug(f"parse_code: {state.parse_code[:32]}")
    logger.debug(f"parse_plan: {state.parse_plan[:32]}")
...
async def node_exec_metrics_parser(state: State, runtime: Runtime[Context]) -> State:
    ...
    logger.debug(f"parse_stdout: {state.parse_stdout[:32]}")
    logger.debug(f"parse_stderr: {state.parse_stderr[:32]}")
