...
def build_s3_download_snippet(
    *,
    ...
    return (
        ...
        "from pathlib import Path\n"
        "from dotenv import load_dotenv\n"
        "from ai_scientist.download_manager import download_file\n"
        "\n"
        ...
        "\n"
        "# Copy a presigned download URL and file size from the 'S3 datasets directory' list above\n"
        "# For large files (>100MB), this uses parallel multipart download automatically\n"
        "download_url = '<PASTE_PRESIGNED_URL_FROM_LIST>'\n"
        "file_size = 12345678  # <-- Copy the file size from the list\n"
        "\n"
        "download_file(\n"
        "    url=download_url,\n"
        "    file_size=file_size,\n"
